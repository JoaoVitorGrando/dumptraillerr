      ...
      <div className="container-page">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="mt-3 sm:mt-4 text-gray-600 text-base sm:text-lg">
            12 to 20 ft hydraulic dump trailers — roofing tear-offs, construction
            debris, full cleanouts and everything in between. Online booking,
            day-before delivery and pickup included.
          </p>
        </div>
        ...
        <p className="mt-6 sm:mt-8 text-center text-sm text-gray-500">
          ...
          <Link
            href="/services/dump-trailer#trailers"
...
function SizeCard({ trailer }: { trailer: Trailer }) {
  const dimension = `${trailer.length.replace(" ft", "")}x${trailer.width.replace(" ft", "")}`;

  return (
    <Link
      href={`/services/dump-trailer?model=${trailer.id}#trailers`}
...
function ComingSoonPreview() {
  const hiddenServices = ALL_SERVICES.filter((s) => s.slug !== "dump-trailer");
  const trackRef = useRef<HTMLUListElement>(null);

  useEffect(() => {
    const el = trackRef.current;
    if (!el || hiddenServices.length <= 1) return;

    const autoAdvance = () => {
      const card = el.querySelector<HTMLElement>("[data-coming-card]");
        ...
        {hiddenServices.map((service) => (
          ...
          >
            ...
            <div className="px-3 pb-3">
              ...
              <p className="mt-0.5 text-[11px] text-gray-500 line-clamp-2">{service.tagline}</p>
            </div>
          </li>
        ))}
      </ul>
