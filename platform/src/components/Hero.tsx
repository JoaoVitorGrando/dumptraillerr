      ...
      <div className="container-page relative grid lg:grid-cols-[minmax(0,1.06fr)_minmax(0,0.94fr)] gap-8 lg:gap-2 items-center">
        <div className="min-w-0 lg:pr-3">
          ...
          <p className="mt-5 sm:mt-6 max-w-3xl text-lg md:text-xl text-white/85 leading-relaxed">
            We connect you to the right trailer for any job — fast, simple, and reliable.
          </p>
          ...
          <div className="mt-7 sm:mt-8 flex flex-col sm:flex-row gap-3">
            <a href="#services" className="btn-primary w-full sm:w-auto !text-sm md:!text-base !px-7">
            ...
            <Link href="/partner" className="btn-outline-light w-full sm:w-auto !text-sm md:!text-base !px-7">
              Partner with FAGU
            </Link>
          </div>

          <div className="mt-8 sm:mt-10 grid grid-cols-3 gap-2 sm:gap-4 max-w-2xl">
            <TrustCard label="For Owners" value="Monetize your trailers with ease" />
            <TrustCard label="For Customers" value="Book fast and get it delivered" />
            <TrustCard label="For Drivers" value="Earn by making local deliveries" />
          </div>
        </div>
...
function TrustCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-white/10 bg-white/5 p-2.5 sm:p-3">
      <p className="text-[10px] sm:text-xs text-white/60 uppercase tracking-wider">{label}</p>
      <p className="font-display text-sm sm:text-lg font-bold leading-tight">{value}</p>
    </div>
  );
}
...
function HeroVisual() {
  const carouselItems = useMemo(() => {
    if (TRAILERS.length) {
      return TRAILERS.map((t) => ({ slug: "dump-trailer", name: t.size, image: t.image }));
    }
    return SERVICES.map((s) => ({ slug: s.slug, name: s.name, image: s.image }));
  }, []);
    ...
    <div className="relative w-full max-w-xl mx-auto lg:max-w-none lg:mt-2 lg:-ml-2">
      ...
      <div className="relative rounded-2xl sm:rounded-3xl border border-white/10 bg-white/[0.03] backdrop-blur-sm p-4 sm:p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3 mb-3 sm:mb-4">
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-[0.18em] text-brand-yellow font-bold">FAGU · Trailer Fleet</p>
            <p className="mt-1 text-white font-display text-xl sm:text-2xl font-extrabold leading-tight">
              Explore our trailer <span className="text-brand-yellow">options.</span>
            </p>
          </div>
          <FaguBadge size="sm" bare className="shrink-0 opacity-90" />
        </div>
        ...
        <div className="relative overflow-hidden rounded-xl border border-white/10">
          {activeItem && (
            <>
              ...
              />
              <div aria-hidden className="absolute inset-0 bg-gradient-to-t from-brand-dark/80 via-brand-dark/15 to-transparent" />
              <div className="absolute left-3 right-3 bottom-3 flex items-center justify-between gap-3 z-20">
                ...
                <div className="flex items-center gap-1.5">
                  {carouselItems.map((item, i) => (
                    <span
                      key={`${item.slug}-${i}`}
                      className={`h-1.5 rounded-full transition-all ${i === activeIndex ? "w-5 bg-brand-yellow" : "w-2.5 bg-white/45"}`}
                      aria-hidden
                    />
                  ))}
                </div>
        ...
        <div className="mt-3 sm:mt-4 flex items-center justify-between gap-3 px-1">
          <div>
            <p className="text-[10px] uppercase tracking-wider text-white/50">Sizes</p>
            <p className="font-semibold text-white text-sm sm:text-base tabular-nums">12 – 20 ft</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] uppercase tracking-wider text-white/50">Delivery</p>
            <p className="font-semibold text-white text-sm sm:text-base">Day-before</p>
          </div>
        </div>

        <Link
          href={activeItem ? `/services/${activeItem.slug}` : "/#services"}
