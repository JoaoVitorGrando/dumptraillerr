...
export default function TrailerCard({ trailer, onSelect }: TrailerCardProps) {
  return (
    <article className="group overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition-all duration-300 hover:shadow-xl hover:border-brand-yellow/60">
      <div className="grid lg:grid-cols-[1.15fr,1fr]">
        <div className="relative bg-brand-dark">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-brand-dark/60 via-brand-dark/20 to-transparent" aria-hidden />

          <div className="absolute top-4 left-4 rounded-xl bg-white/95 backdrop-blur px-3 py-2 shadow">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/assets/LOGO1.png" alt="FAGU logo" loading="lazy" className="h-8 w-auto object-contain" />
          </div>

          <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-3 text-white">
            <div>
              <p className="text-[10px] uppercase tracking-[0.16em] text-white/75 font-semibold">Selected Size</p>
              <p className="text-3xl sm:text-4xl font-bold tabular-nums">{trailer.length}</p>
            </div>
        ...
        <div className="flex flex-col p-5 sm:p-6">
          ...
          <p className="mt-1 text-brand-orange font-semibold text-sm sm:text-base">{trailer.bestFor}</p>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between gap-3 sm:gap-4 border-b border-gray-100 pb-2">
      <dt className="text-gray-500 uppercase text-[11px] sm:text-xs tracking-wider whitespace-nowrap">{label}</dt>
      <dd className="text-right text-brand-dark font-semibold break-words min-w-0">{value}</dd>
    </div>
  );
}
