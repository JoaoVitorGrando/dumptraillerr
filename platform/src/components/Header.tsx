    ...
    <header className="fixed top-0 inset-x-0 z-50 transition-all duration-300">
      ...
      >
        <div className="container-page flex items-center justify-center gap-2 sm:gap-3 py-1.5 sm:py-2 text-center">
          ...
          <p className="promo-copy-attention text-[11px] sm:text-sm font-semibold leading-tight">
            <span className="font-extrabold uppercase">Save 50%</span> on your second dump
            trailer for the same job site.{" "}
            <Link
              href="/services/dump-trailer#rules"
      ...
      <div className={`bg-brand-dark/75 backdrop-blur-md transition-shadow duration-300 ${scrolled ? "shadow-lg bg-brand-dark/80" : ""}`}>
        <div className="container-page flex h-24 md:h-28 items-center justify-between gap-3">
          <Link href="/" className="flex items-center text-white min-w-0 flex-1" aria-label="FAGU Home Services — Home">
            <FaguBadge size="xl" bare className="!h-20 !w-20 sm:!h-28 sm:!w-28 md:!h-32 md:!w-32" />
          </Link>

          {/* Desktop nav */}
          <nav className="hidden lg:flex items-center gap-5 xl:gap-7">
            <Link href="/" className="text-white/90 hover:text-brand-yellow font-medium transition-colors">
              Home
            </Link>

            {isSingleService ? (
              <Link href={`/services/${PUBLIC_SERVICES[0].slug}`} className="text-white/90 hover:text-brand-yellow font-medium transition-colors">
                Dump Trailers
              </Link>
            ) : (
              <div
            ...
            {NAV_LINKS.filter((l) => l.to !== "/").map((l) => (
              <Link key={l.to} href={l.to} className="text-white/90 hover:text-brand-yellow font-medium transition-colors">
                {l.label}
              </Link>
            ))}

            <a href={RESERVE_HREF} className="btn-primary !py-2.5 !px-5 !text-xs xl:!text-sm">
              Reserve a Trailer
            </a>
          </nav>
      ...
      <div className={`lg:hidden overflow-hidden bg-brand-dark border-t border-white/10 transition-[max-height] duration-300 ${open ? "max-h-[44rem]" : "max-h-0"}`}>
        <nav className="container-page flex flex-col py-3">
          <Link href="/" className="py-3 text-white/90 border-b border-white/5 hover:text-brand-yellow text-base font-semibold">
            Home
          </Link>

          {isSingleService ? (
            <Link href={`/services/${PUBLIC_SERVICES[0].slug}`} className="py-3 text-white/90 border-b border-white/5 hover:text-brand-yellow text-base font-semibold">
              Dump Trailers
            </Link>
          ) : (
            <div className="py-3 border-b border-white/5">
              ...
              <ul className="grid grid-cols-1 gap-1">
                {PUBLIC_SERVICES.map((s) => (
                  <li key={s.slug}>
                    <Link href={`/services/${s.slug}`} className="flex items-center justify-between rounded-md px-2 py-2 text-white/85 hover:bg-white/5 hover:text-brand-yellow">
                      <span>{s.name}</span>
                      <span className={`text-[10px] font-bold uppercase tracking-wider ${s.available ? "text-brand-yellow" : "text-white/40"}`}>
                        {s.available ? "Live" : "Soon"}
                      </span>
          ...
          {NAV_LINKS.filter((l) => l.to !== "/").map((l) => (
            <Link key={l.to} href={l.to} className="py-3 text-white/90 border-b border-white/5 hover:text-brand-yellow text-base">
              {l.label}
            </Link>
          ))}

          <a href={RESERVE_HREF} onClick={() => setOpen(false)} className="btn-primary mt-3 w-full">
            Reserve a Trailer
          </a>
        </nav>
      </div>
